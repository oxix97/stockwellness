...
/**
 * API: 금융위원회_KRX상장종목정보 (GetKrxListedInfoService)
 * 기능: 종목정보 (getItemInfo)
 * 문서 참조:
 */
...
public record KrxListedInfoResponse(
    ...
    @JsonIgnoreProperties(ignoreUnknown = true) // 추후 API 필드 추가 시 에러 방지
    public record Item(
            @JsonProperty("basDt")
    ...
    public List<KrxListedInfoResponse.Item> getItems() {
