...
- [ ] **Task: 목표 비중 도메인 로직 추가**
    - [ ] `PortfolioItem` 또는 별도 테이블에 `targetWeight` 필드 추가
    - [ ] 포트폴리오 수준의 목표 비중 합계 검증 로직 (100% 체크)
- [ ] **Task: 리밸런싱 엔진 구현**
    - [ ] 현재 시장 가치 기반 실시간 비중 계산 로직 고도화
    - [ ] 목표 비중 도달을 위한 필요 매매 수량(Buy/Sell Quantity) 계산 유틸리티 구현
- [ ] **Task: 리밸런싱 API 개발**
