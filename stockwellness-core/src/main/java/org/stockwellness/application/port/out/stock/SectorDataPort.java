...
public interface SectorDataPort {
    /**
     * 특정 업종 코드의 당일 상세 지수 정보를 가져옵니다.
     */
    KisDailySectorDetail fetchDailySectorDetail(String indexCode);

    /**
     * 특정 업종 코드의 최근 N일간 투자자 매매동향을 가져옵니다.
     */
    List<InvestorTradingDaily> fetchInvestorTradingDaily(String indexCode, int days);

    /**
     * 특정 업종 코드의 과거 지수 이력을 가져옵니다. (MA, RSI 계산용)
     */
    List<BigDecimal> fetchHistoricalIndexPrices(String indexCode, int days);
