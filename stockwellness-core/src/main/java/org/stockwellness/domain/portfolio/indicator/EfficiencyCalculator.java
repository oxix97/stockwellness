...
public class EfficiencyCalculator implements IndicatorCalculator<EfficiencyCalculator.EfficiencyMetrics> {
    ...
    public EfficiencyMetrics calculate(IndicatorContext context) {
        ...
        if (annualizedVolatility.compareTo(BigDecimal.ZERO) > 0) {
            sharpeRatio = excessReturn.divide(annualizedVolatility, 4, RoundingMode.HALF_UP);
        }
        ...
        if (annualizedDownsideVolatility.compareTo(BigDecimal.ZERO) > 0) {
            sortinoRatio = excessReturn.divide(annualizedDownsideVolatility, 4, RoundingMode.HALF_UP);
        }
    ...
    private BigDecimal calculateDownsideVolatility(List<BigDecimal> returns) {
        ...
        return FinancialMath.sqrt(downsideVariance).setScale(4, RoundingMode.HALF_UP);
    }
}
