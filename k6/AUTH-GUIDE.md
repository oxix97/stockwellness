...
`run-all.sh`는 네 가지 읽기 테스트 모드를 지원합니다.

### 2.1 Quick 모드 (기능 및 인증 검증)
시나리오당 **10초**만 실행하여 인증 여부 및 API의 정상 동작을 빠르게 확인합니다.
```bash
./k6/run-all.sh quick
```

### 2.2 Short 모드 (빠른 성능 회귀 확인)
시나리오당 약 **2분** 실행하여 짧은 성능 회귀를 확인합니다.
```bash
./k6/run-all.sh short
```

### 2.3 Standard 모드 (성과 기록용 부하 측정)
시나리오당 약 **5분** 실행하며, 이력서나 보고서에 사용할 성능 지표를 측정합니다.
```bash
./k6/run-all.sh standard
```

### 2.4 Stress 모드 (고부하 안정성 확인)
시나리오당 약 **10분** 실행하며, tail latency와 장시간 고부하 안정성을 확인합니다.
```bash
./k6/run-all.sh stress
```
