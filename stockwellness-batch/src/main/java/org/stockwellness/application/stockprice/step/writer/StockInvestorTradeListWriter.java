package org.stockwellness.application.stockprice.step.writer;
...
public class StockInvestorTradeListWriter implements ItemWriter<List<StockInvestorTrade>> {

    private final JdbcTemplate jdbcTemplate;

    @Override
    public void write(Chunk<? extends List<StockInvestorTrade>> chunk) {
        List<StockInvestorTrade> trades = new ArrayList<>();

        for (List<StockInvestorTrade> items : chunk) {
            if (items != null) {
                trades.addAll(items);
            }
        }
